...
let isComputerTurn = false;
  ...
  // Modify your existing switchTurns function to call the computerMove for player2
  ...
  function computerMove() {
    ...
    const capturingPieces = Array.from(pieces).filter(piece => {
      ...
      return (
        (row + 2 < checkerBoard.size && col + 2 < checkerBoard.size && !checkerBoard.hasPieceAt(row + 2, col + 2)) ||
        (row + 2 < checkerBoard.size && col - 2 >= 0 && !checkerBoard.hasPieceAt(row + 2, col - 2))
      );
    });
...
class CheckerBoard {
  ...
  checkForWinner() {
    ...
    if (blackPieceCount === 0) {
      alert("Computer wins!"); // Display an alert or update the UI to indicate the winner
      this.restart(); // Restart the game after displaying the winner
    } else if (redPieceCount === 0) {
      alert("Player wins!"); // Display an alert or update the UI to indicate the winner
      this.restart(); // Restart the game after displaying the winner
    }
  }
