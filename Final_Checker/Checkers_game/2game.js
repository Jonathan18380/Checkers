//All comments on game.js or singlegame.js besides copy/pasted comments.
...
function createCheckerBoard(size) {
  ...
  player1.startTurn(); 
  checkerBoard = new CheckerBoard(size); //Creates the board based on the HTML option selected (8x8 or 10x10)
}
...
//highlightMove function to handle capturing
...
class CheckerBoard {
  ...
  checkForWinner() {
    ...
    if (blackPieceCount === 0) {
      alert("Player 2 wins!"); // Display an alert or update the UI to indicate the winner
      this.restart(); // Restart the game after displaying the winner
    } else if (redPieceCount === 0) {
      alert("Player 1 wins!"); // Display an alert or update the UI to indicate the winner
      this.restart(); // Restart the game after displaying the winner
    }
  }
